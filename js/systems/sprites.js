...
// Dynamic sprite definitions, populated by registerSpriteDefs()
let spriteDefs = {};
...
/**
 * Register sprite definitions from unit configs.
 * Called by initRegistry() — pulls spriteSheet from each unit that has one.
 */
export function registerSpriteDefs(unitTypes) {
  spriteDefs = {};
  for (const unit of Object.values(unitTypes)) {
    if (unit.spriteSheet) {
      spriteDefs[unit.type] = unit.spriteSheet;
    }
  }
}
...
export function loadSprites() {
  const promises = Object.entries(spriteDefs).map(([type, def]) => {
...
export function getSpriteDef(unitType) {
  return spriteDefs[unitType] || null;
}
...
export function getSpriteFrame(unit, state) {
  ...
  const def = spriteDefs[unit.type];
  if (!def) return null;
